...
import { displayStatus, isDoneCleared, progressColor } from "@/lib/domain/status";
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <Section title="Status & Progress">
            ...
            <>
              ...
              <FieldRow label={`Progress · ${node.progress}%`}>
                <input type="range" min={0} max={100} value={node.progress} disabled={!canEdit}
                  onChange={(e) => changeProgress(Number(e.target.value))} className="w-full accent-brand-blue" />
              </FieldRow>
            </>
          ) : (
          ...
          <FieldRow label={`Volume (weight) · ${node.volume}`}>
            <input type="range" min={1} max={10} value={node.volume} disabled={!canEdit}
              onChange={(e) => patch({ volume: Number(e.target.value) })} className="w-full accent-brand-orange" />
          </FieldRow>
          ...
          {node.startDate && node.dueDate && (
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded bg-surface-2" title={`${node.startDate} → ${node.dueDate}`}>
        ...
        {/* 2. Gates */}
        <Section title="Gates (QA / HSE)">
          <FieldRow label="QA gate">
            {canQa ? (
        ...
        {/* 3. Notes & RFI */}
        <Section title="Notes & RFI" right={<span className="count-badge">{node.notes.length}</span>}>
          <NotesSection node={node} canEdit={canEdit} />
        </Section>

        {/* 4. Dependencies */}
        <Section title="Dependencies" right={<span className="count-badge">{node.dependencies.length}</span>}>
        ...
        <Section title="Info" defaultOpen={false}>
          ...
          <FieldRow label="Category">
            <Select value={node.category} disabled={!canEdit} onChange={(e) => patch({ category: e.target.value }, { type: "category", message: `${node.nodeCode} → ${e.target.value}` })}
              options={[...new Set([node.category, ...t.categories.map((c) => c.name)])].map((c) => ({ value: c, label: c }))} />
          </FieldRow>
        ...
        <Section title="Assignee" defaultOpen={false}>
          {members.length > 0 && (
            <FieldRow label="Assign to member">
              <Select value={node.assignedUserId ?? ""} disabled={!canEdit}
                onChange={(e) => {
                  const m = members.find((x) => x.userId === e.target.value);
                  void patch({ assignedUserId: e.target.value || null, assignee: m ? { name: m.name, email: m.email } : undefined }, { type: "assignee", message: `${node.nodeCode} assigned` });
                }}
                options={[{ value: "", label: "— External / none —" }, ...members.map((m) => ({ value: m.userId, label: m.name || m.email }))]} />
            </FieldRow>
          )}
          ...
          {(node.assignee.email || node.assignee.phone) && (
            <div className="flex gap-2">
              {node.assignee.email && <a href={`mailto:${node.assignee.email}`} className="inline-flex items-center gap-1 rounded border border-line px-2 py-1 text-2xs text-ink-dim hover:text-ink"><Mail size={12} />Email</a>}
              {node.assignee.phone && <a href={`tel:${node.assignee.phone}`} className="inline-flex items-center gap-1 rounded border border-line px-2 py-1 text-2xs text-ink-dim hover:text-ink"><Phone size={12} />Call</a>}
            </div>
          )}
        </Section>
      </div>
