...
import {
  Plus, Trash2, Paperclip, Check, RotateCcw, X,
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-end gap-1">
        {(["view", "manage"] as const).map((m) => {
          const active = (m === "manage") === manage;
          if (m === "manage" && !canNote) return null;
          return (
            <button
              key={m}
              onClick={() => setManage(m === "manage")}
              className={clsx("rounded px-2 py-0.5 font-mono text-2xs uppercase tracking-widest", active ? "bg-brand-blue text-white" : "text-ink-mute hover:text-ink")}
            >
              {m}
            </button>
          );
              ...
              <li key={n.id} className="rounded border border-line bg-canvas px-2 py-1.5">
                {manage ? (
                  <Input
                    ...
                    onBlur={(e) => e.target.value !== n.text && t.updateNote(node.id, n.id, { text: e.target.value }).catch((err) => toast.error(errMessage(err)))}
                  />
                ) : (
                  <p className={clsx("text-sm", n.checked ? "text-brand-green line-through" : "text-ink")}>{n.text}</p>
                )}

                {n.attachments.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {n.attachments.map((a) => (
                      <AttachmentChip key={a.id} att={a} onRemove={manage ? () => t.removeAttachment(node.id, n.id, a).catch((e) => toast.error(errMessage(e))) : undefined} />
                    ))}
                  </div>
                )}
      ...
      {canNote && (
        <div className="space-y-1.5 rounded border border-line bg-canvas p-2">
          <Input value={src} onChange={(e) => setSrc(e.target.value)} placeholder="Source (e.g. contractor, site, QA)" className="h-7 text-sm" />
          <textarea
            value={text}
